  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h2 className="font-serif text-xl text-brand-white">Order History</h2>
      ...
      <div className="space-y-4">
        {filtered.length === 0 ? (
          <Card className="p-6 text-sm text-white/50">No orders match this filter.</Card>
        ) : (
          filtered.map((order) => (
            <Card key={order.id} className="p-5">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="text-xs text-white/40">Placed {formatDate(order.date)}</p>
                </div>
                <Badge variant={statusVariant(order.status)}>{order.status}</Badge>
              </div>
              <div className="mt-4 flex flex-wrap items-center gap-4 border-t border-white/10 pt-4 text-sm">
                <span className="text-white/60">{order.items.length} item(s)</span>
