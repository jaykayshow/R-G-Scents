  ...
  return (
    <article className="mx-auto max-w-3xl px-4 py-20 sm:px-6">
      <nav className="mb-8 text-xs text-overlay/40">
        <Link href="/blog" className="hover:text-gold">Journal</Link> / <span className="text-overlay/60">{post.title}</span>
      </nav>
      <Eyebrow>{post.category}</Eyebrow>
      <h1 className="mt-3 font-serif text-3xl font-semibold text-fg sm:text-4xl">{post.title}</h1>
      <p className="mt-3 text-xs text-overlay/40">{post.author} · {formatDate(post.date)}</p>
