        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
          {galleryPosts.map((post) => (
            <button
              ...
              className="group relative aspect-square overflow-hidden rounded-md bg-matte-black"
            >
              <Image
                src={post.image}
                alt={post.caption}
              ...
              />
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-matte-black/0 opacity-0 transition-all duration-300 group-hover:bg-matte-black/70 group-hover:opacity-100">
                <Heart size={18} className="fill-gold text-gold" />
                <span className="text-xs text-brand-white">{post.likes}</span>
              </div>
            </button>
          ))}
        </div>
